...
gdpPercap_coeff <- lm["gdpPercap", "Estimate"]
gdpPercap_p_val <- lm["gdpPercap", "Pr(>|t|)"]

actual_fitted_df <- data.frame("actual" = my_gapminder$lifeExp, "fitted" = lm_fit$ybar)
ggplot(data = actual_fitted_df, aes(x = fitted, y = actual)) + geom_point()
```
While the `gdpPercap` coefficient is very small (`gdpPercap_coeff`), it is still significant. The null hypothesis for the `gdpPercap` coefficient is that it does affect life expectancy significantly. The alternative hypothesis is that `gdpPercap` does affect life expectancy significantly. The p-value for the gdpPercap coefficient is `gdpPercap_p_val` which is much below the p-value cut off of 0.05. Thus, there is sufficient evidence to reject the null hypothesis and conclude that gdp per capita affects life expectancy. 
